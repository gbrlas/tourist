...
<div class="container">
    ...
    if (isset($_POST['accomodationId'])) {
        $id = $_POST['accomodationId'];
    }
    ...
    if (! isset($_GET['sure'])) {
        ...
        echo "<div class=\"row\">
            ...
            <h3>Jeste li sigurni da želite obrisati smještaj broj <b>$id</b>?</h3>

            <a class=\"btn btn-danger\" href=\"./removeAccomodations.php?sure=true&value=$id\">Obriši smještaj<span class=\"glyphicon glyphicon-chevron-right\"></span></a>
        ...
        $upit = "DELETE FROM slike_smjestaj WHERE idSmjestaj = $id";
        mysqli_query($veza, $upit) or die ("1" . mysqli_error($veza));

        $upit = "SELECT tip FROM smjestaj WHERE idSmjestaj = $id";
        $rezultat = mysqli_query($veza, $upit) or die ("1" . mysqli_error($veza));
        $redak = mysqli_fetch_array($rezultat, MYSQLI_ASSOC);
        $tip = $redak['tip'];

        if ($tip == 1)  {
            $upit = "DELETE FROM hotel WHERE idSmjestaj = $id";
            mysqli_query($veza, $upit) or die ("1" . mysqli_error($veza));
        } else if ($tip == 2) {
            $upit = "DELETE FROM apartman WHERE idSmjestaj = $id";
            mysqli_query($veza, $upit) or die ("1" . mysqli_error($veza));
        }

        $upit = "DELETE FROM smjestaj WHERE idSmjestaj = $id";
        mysqli_query($veza, $upit) or die ("1" . mysqli_error($veza));


        echo "<div class=\"row\">
                    <div class=\"col-md-6\">
                        <h4 style='color: limegreen'>Lokacija <b>$id</b> uspješno obrisan. </h4></div></div>";
    }
    ?>
