...
<head>
    ...
    <title>TRAVEL TIME ADMIN - Uređivanje smještaja</title>
...
<div class="container">
    ...
    if (! isset($_GET['changed'])) {
        ...
        <form class=\"form-horizontal\" action=\"./editAccomodations.php?changed=true&value=$id\" method=\"post\">
        <div class=\"form-group\">
            ...
            <div class=\"form-group\">
                ...
                <div class=\"col-sm-9\">
                    <input name=\"address\" class=\"form-control\" id=\"address\" value=\"$adresa\">
                </div>
            </div>
            <div class=\"form-group\">
                <label for=\"classification\" class=\"col-sm-2 control-label\">Klasifikacija</label>
                <div class=\"col-sm-2\">
                    <input name=\"classification\" class=\"form-control\" id=\"classification\" value=\"$klasifikacija\">
                </div>
            </div>
            <div class=\"form-group\">
        ...
        $opis = navodnici($_POST['description']);
        $tip = $_POST['type'];
        $adresa = navodnici($_POST['address']);
        $klasifikacija = $_POST['classification'];
        $lokacija = $_POST['location'];
        $discount = $_POST['discount'];

        $discount = !empty($discount) ? "$discount" : "NULL";

        $upit = "UPDATE smjestaj SET tip = $tip, opis = $opis, adresa = $adresa, klasifikacija = $klasifikacija, idLokacija = $lokacija, idAkcija = $discount WHERE idSmjestaj = $id";
        mysqli_query($veza, $upit) or die ("2" . mysqli_error($veza));

        echo "<div class=\"row\">
                    <div class=\"col-md-6\">
                        <h4 style='color: limegreen'>Smještaj <b>$id</b> uspješno uređen. </h4></div></div>";
    }
    ?>
