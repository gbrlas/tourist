...
<head>
    ...
    <title>TRAVEL TIME ADMIN - Brisanje izleta</title>
...
<div class="container">
    ...
    if (! isset($_GET['sure'])) {
        ...
        echo "<div class=\"row\">
            ...
            <h3>Jeste li sigurni da želite obrisati izlet broj <b>$id</b>?</h3>

            <a class=\"btn btn-danger\" href=\"./removeTours.php?sure=true&value=$id\">Obriši izlet<span class=\"glyphicon glyphicon-chevron-right\"></span></a>
        ...
        $upit = "DELETE FROM slike_izlet WHERE idIzlet = $id";
        mysqli_query($veza, $upit) or die ("1" . mysqli_error($veza));

        $upit = "DELETE FROM izlet_polazak WHERE idLokacija = $id";
        mysqli_query($veza, $upit) or die ("2" . mysqli_error($veza));

        $upit = "DELETE FROM izlet WHERE idLokacija = $id";
        mysqli_query($veza, $upit) or die ("3" . mysqli_error($veza));

        echo "<div class=\"row\">
                    <div class=\"col-md-6\">
                        <h4 style='color: limegreen'>Izlet <b>$id</b> uspješno obrisan. </h4></div></div>";
    }
    ?>
